    ...
    public class RecurringJobsFromConfiguration
    {
        ...
        public void UpdateJobs()
        {
            ...
            var oldJobs = new HashSet<string>(_backgroundJobs
                .ListRecurringJobs()
                .Where(name => name.EndsWith(JobNameSuffix)));
            ...
            foreach (var job in newJobs)
            {
                ...
                if (actionType == null)
                    throw new ArgumentException($"Invalid configuration settings in {ConfigurationProvider.GetKey<RecurringJobsOptions, object>(o => o.Recurring)}:" +
        ...
        private void CreteCustomLock(string key)
        {
            ...
            catch (FrameworkException ex) when (ex.Message.TrimEnd().EndsWith($"{key} lock."))
            {
