...
namespace Rhetos.Jobs.Hangfire
{
    ...
    public class RecurringJobsOptions
    {
        ...
        {
            var recurringKeyPrefix = ConfigurationProvider.GetKey<RecurringJobsOptions, object>(o => o.Recurring)
                + ConfigurationProvider.ConfigurationPathSeparator;

            var recurringJobsNames = configuration.AllKeys
                .Where(key => key.StartsWith(recurringKeyPrefix))
                .Select(key => key
                    .Substring(recurringKeyPrefix.Length)
                    .Split(new[] { ConfigurationProvider.ConfigurationPathSeparator }, System.StringSplitOptions.None)
                    .First())
                .Distinct()
                .ToList();
